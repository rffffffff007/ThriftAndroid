...
public class THttpClient extends TTransport {
	...
	public THttpClient(String url, HttpClient client)
			throws TTransportException {
		try {
			url_ = new URL(url);
			this.client = client;
			this.host = new HttpHost(url_.getHost(),
					-1 == url_.getPort() ? url_.getDefaultPort()
	...
	private void flushUsingHttpClient() throws TTransportException {
		...
		try {
			...
			// added by renfei. gzip added.
			if (isEncodingGZIP(response)) {
			...
			if (headers != null) {
				...
				for (Header h : headers) {
					List<String> values = map.get(h.getName());
					if (values == null) {
						values = new ArrayList<String>();
						map.put(h.getName(), values);
					}
			...
			try {
				...
				// Modified for Android, EntityUtils.consume doesn't exist in
				// Android.
	...
	public List<String> getResponseHeader(String name) {
		if (mResponseHeaders == null || name == null)
			return null;
